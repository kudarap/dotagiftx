...
import FixSteamIDDialog from '@/components/FixSteamIDDialog'
import type { Market, Item } from '@/lib/types'

type ActivityMarket = Market & { item: Item }
...
interface FixMyMarketActivityProps {
  datatable: { data: ActivityMarket[] }
  loading: boolean
  error: string | null
...
export default function FixMyMarketActivity({
  datatable,
  loading,
  error,
  onSearchInput,
  onReload,
}: FixMyMarketActivityProps) {
  const { classes } = useStyles()

  const [currentMarket, setCurrentMarket] = React.useState<ActivityMarket | null>(null)
  const handleUpdateClick = (marketIdx: number | null) => {
    if (marketIdx === null) {
  ...
  return (
    <>
      ...
      <ul className={classes.list}>
        {datatable.data.map((market, marketIdx) => (
