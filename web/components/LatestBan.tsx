...
const getDaysFromTs = (datetime: string) => {
...
export default function LatestBan() {
  const [value, setValue] = useState<string>(() => get(CACHE_KEY) || '')
  ...
  React.useEffect(() => {
    ;(async () => {
      ...
      try {
        const res = (await blacklistSearch({
          limit: 1,
  ...
  const grayscale = Number((getDaysFromTs(value) / sinceRate).toFixed(2)) * 100 || 0
  return (
    <span
      style={{
        position: 'absolute',
