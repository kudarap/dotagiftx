...
const marketSalesGraphFilter: QueryFilter = {
  type: MARKET_TYPE_ASK,
}

const marketReservedFilter: QueryFilter = {
  type: MARKET_TYPE_ASK,
...
const marketDeliveredFilter: QueryFilter = {
  type: MARKET_TYPE_ASK,
...
function ItemGraph({ itemId = '', itemName = '' }: { itemId?: string; itemName?: string }) {
  // Retrieve market sales graph.
  const shouldLoadGraph = true
  marketSalesGraphFilter.item_id = itemId
  const { data: marketGraph, error: marketGraphError } = useSWR<MarketSalePoint[]>(
  ...
  } = useSWR<MarketSearchResponse>(
    shouldLoadHistory ? [MARKETS, marketReservedFilter] : null,
    ...swrConfig
  )
  ...
  const {
    ...
    marketReserved ? [MARKETS, marketDeliveredFilter] : null,
    ...swrConfig
  )

  let historyCount: number | false = false
  if (!marketReservedError && marketReserved) {
    historyCount = marketReserved.total_count
  }
  if (!marketDeliveredError && marketDelivered) {
    historyCount = (historyCount === false ? 0 : historyCount) + marketDelivered.total_count
  }
