...
const validateSteamProfileURL = (rawURL: string) => {
...
interface FixSteamIDDialogProps {
  market: Market | null
  open?: boolean
  onClose?: () => void
  onCancel?: () => void
...
export default function FixSteamIDDialog(props: FixSteamIDDialogProps) {
  const { classes } = useStyles()
  const { isMobile } = useContext(AppContext)

  const { market, onSuccess = () => {}, onCancel = () => {}, onClose = () => {} } = props
  ...
  const marketUpdate = (
    payload: { status?: number; partner_steam_id: string; notes: string },
