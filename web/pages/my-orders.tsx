...
const initialMarketStats: StatsSummary = {
  pending: 0,
  live: 0,
  reserved: 0,
  sold: 0,
...
export default function MyOrders() {
  ...
  const [tabValue, setTabValue] = React.useState<string | false>(false)
  ...
  React.useEffect(() => {
    ;(async () => {
      const res = (await statsMarketSummary({
        ...
        index: 'user_id',
      })) as StatsSummary & { bids: StatsSummary }
      ...
      })) as StatsSummary & { reserved: number }
  ...
  const handleTabChange = (e: unknown, v: string) => {
    setTabValue(v)
    router.push(v)
  }
