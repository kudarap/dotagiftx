...
function ButtonWrapper({ planId, onSuccess }) {
  return (
    <PayPalSubscriptionButton
      createSubscription={async () => {
        const res = await createMySubscription(planId)
...
export default function Checkout() {
  ...
  const cardStyle = subscriptionStyleCard[query.id]
  const [currentProfile, setCurrentProfile] = React.useState(defaultProfile)
  useEffect(() => {
    if (!query.id) {
      return
    }
    ...
    ;(async () => {
      const res = await myProfile.GET(true)
      setCurrentProfile({
        ...res,
