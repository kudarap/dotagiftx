...
const backlightColor: Record<string, string> = {
  partner: '#ffab00',
  trader: '#06a5ff',
  supporter: '#034eff',
}
...
export default function ThanksSubscriber() {
  ...
  React.useEffect(() => {
    ;(async () => {
      ...
      const remote = (await myProfile.GET()) as Profile
      Storage.save(APP_CACHE_PROFILE, remote)
      setProfile(remote)
    })()
  }, [isLoggedIn])

  const [subscription, setSubscription] = React.useState<string | boolean | null>(null)
  React.useEffect(() => {
    if (!subscriptionID) {
      return
    }
  ...
  const badge =
    subscription === true || subscription === false || subscription === null
      ? undefined
      : (subscription as SubscriberBadgeType | undefined)
          ...
          {profile && subscriptionID && (
            <Box sx={{ mt: 8, textAlign: 'center' }}>
              ...
              {typeof subscription === 'string' && (
                <SubscriberBadge type={subscription as SubscriberBadgeType} size="large" />
              )}
          ...
          <Divider
            sx={{
              ...
              boxShadow: subscription
                ? `0 -60px 290px ${backlightColor[subscription as string]}`
                : `0 -60px 290px black`,
            }}
          />
