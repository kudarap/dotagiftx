...
interface BlacklistUser extends UserSummary {
  status: number
  updated_at: string
}
...
export default function Blacklist() {
  ...
  const { data, error } = useSWR<BlacklistUser[]>(url, fetcherBase)

  const router = useRouter()
  let resolvedQuery: string | false = false
  if (startsWith(query, STEAMURL, 0)) {
    resolvedQuery = resolveProfileURL(query)
    if (isVanityURL(query)) {
      router.push(resolvedQuery as string)
    }
  }
...
} & Omit<React.ComponentProps<typeof TextField>, 'value' | 'onInput'>) {
...
function UserCard({ data }: { data: BlacklistUser }) {
  return (
