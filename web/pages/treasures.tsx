...
const isTreasureNew = (v: string | null | undefined) => {
  const releaseDate = new Date(v as string)
  if (!releaseDate) {
    return false
  }

  const now = new Date()
  const diff = (now.getTime() - releaseDate.getTime()) / (1000 * 3600 * 24)
  return diff < STILL_NEW_DAYS
}

export const isRecentTreasureNew = () => isTreasureNew(LATEST_TREASURE_DROP.toISOString())
...
interface TreasurePageItem extends Treasure {
  rarity?: string
  release_date?: string
}
      ...
      <main>
        <div
          ...
          <div
            style={{
              // background:
              // 'url(https://cdn.steamstatic.com/steam/apps/570/library_hero.jpg?t=1724395576617) no-repeat center center',
              background:
                'url(https://cdn.steamstatic.com/apps/dota2/images/dota_react/darkcarnival/backgrounds/sky_backdrop.png) no-repeat center top',
...
export const getStaticProps = async () => {
  const res = (await treasureList()) as TreasurePageItem[]
  return {
    props: {
      treasures: res,
      error: null,
