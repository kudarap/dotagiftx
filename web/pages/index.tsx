...
export default function Index({
  ...
  trendingItems: { error: unknown; data?: Item[] }
}) {
  const { classes } = useStyles()

  const { data: recentBidItems, error: recentBidError } = useSWR<SearchResponse<Item>>(
    [CATALOGS, recentBidItemsFilter],
    fetcher
  )
  const { data: recentItems, error: recentError } = useSWR<SearchResponse<Item>>(
    recentBidItems ? [CATALOGS, recentItemsFilter] : null,
    fetcher
  )
  const { data: topSellers } = useSWR<SearchResponse<Item>>(
    recentItems ? [CATALOGS, topSellerItemsFilter] : null,
    fetcher
  )
  const { data: topOrigins } = useSWR<string[]>(topSellers ? STATS_TOP_ORIGINS : null, fetcher)
  const { data: topHeroes } = useSWR<string[]>(topOrigins ? STATS_TOP_HEROES : null, fetcher)
    ...
    <>
      ...
      <main className={classes.main}>
        ...
        <Container
          ...
          {trendingItems.error && (
            <div>failed to load trending items: {String(trendingItems.error)}</div>
          )}
...
export async function getServerSideProps() {
  ...
  let trendingItems: Partial<SearchResponse<Item>> & { error?: unknown } = { error: null }
  try {
    trendingItems = (await catalogTrendSearch()) as SearchResponse<Item>
  } catch (e) {
