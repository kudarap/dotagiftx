...
func (s *Server) publicRouter(r chi.Router) {
	r.Group(func(r chi.Router) {
		...
		r.Get("/users/{id}", handlePublicProfile(s.userSvc, s.cache, s.logger))
		r.Get("/t", handleTracker(s.trackSvc, s.logger))
		r.Get("/sitemap.xml", handleSitemap(s.itemSvc))
		r.Get("/stats/market_summary_v2", handleStatsMarketSummaryV2(s.statsSvc, s.cache, s.logger))
		r.Get("/stats/market_summary_overall", handleStatsMarketSummaryOverall(s.statsSvc, s.cache, s.logger))
		r.Get("/stats/top_origins", handleStatsTopOrigins(s.itemSvc, s.cache, s.logger))
		r.Get("/stats/top_heroes", handleStatsTopHeroes(s.itemSvc, s.cache, s.logger))
		r.Get("/stats/top_keywords", handleStatsTopKeywords(s.statsSvc, s.cache, s.logger))
		r.Get("/graph/market_sales", handleGraphMarketSales(s.statsSvc, s.cache, s.logger))
		r.Route("/reports", func(r chi.Router) {
			r.Get("/", handleReportList(s.reportSvc))
			r.Get("/{id}", handleReportDetail(s.reportSvc))
		})
		r.Get("/vanity/{id}", handleVanityProfile(s.userSvc, s.steam, s.cache, s.logger))
		r.Get("/blacklists", handleBlacklisted(s.userSvc, s.cache, s.logger))
...
func (s *Server) privateRouter(r chi.Router) {
	r.Group(func(r chi.Router) {
		...
		r.Route("/my", func(r chi.Router) {
			r.Get("/profile", handleProfile(s.userSvc, s.cache, s.logger))
			r.Route("/markets", func(r chi.Router) {
				r.Get("/", handleMarketList(s.marketSvc, s.trackSvc, true, s.cache, s.logger))
				r.Post("/", handleMarketCreate(s.marketSvc, s.cache, s.logger))
				r.Get("/{id}", handleMarketDetail(s.marketSvc, s.cache, s.logger))
				r.Patch("/{id}", handleMarketUpdate(s.marketSvc, s.cache, s.logger))
			})
			r.Route("/subscription", func(r chi.Router) {
				r.Post("/create", handleCreateSubscription(s.userSvc))
				r.Post("/process", handleProcSubscription(s.userSvc, s.cache, s.logger))
			})
		})
		r.Post("/items", handleItemCreate(s.itemSvc, s.cache, s.divineKey, s.logger))
		r.Post("/items_import", handleItemImport(s.itemSvc, s.cache, s.divineKey, s.logger))
		r.Post("/images", handleImageUpload(s.imageSvc, s.logger))
		r.Post("/reports", handleReportCreate(s.reportSvc))
		r.Post("/hammer/ban", handleHammerBan(s.hammerSvc, s.cache, s.logger))
		r.Post("/hammer/suspend", handleHammerSuspend(s.hammerSvc, s.cache, s.logger))
		r.Post("/hammer/lift", handleHammerLift(s.hammerSvc, s.cache, s.logger))
		r.Post("/subscription", handleUserManualSubscription(s.userSvc, s.cache, s.divineKey, s.logger))
	})
}
