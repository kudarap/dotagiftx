package rethink

import (
	"context"
	"errors"
	"fmt"
	"log"

	"dario.cat/mergo"
	dotagiftx2 "github.com/kudarap/dotagiftx/dotagiftx"
	r "gopkg.in/rethinkdb/rethinkdb-go.v6"
)

const (
	tableItem     = "item"
	itemFieldName = "name"
	itemFieldSlug = "slug"
)

var itemSearchFields = []string{"name", "hero", "origin", "rarity"}

// NewItem creates new instance of item data store.
func NewItem(c *Client) *ItemRepository {
	ctx := context.Background()
	if err := c.autoMigrate(ctx, tableItem); err != nil {
		log.Fatalf("could not create %s table: %s", tableItem, err)
	}

	if err := c.createIndex(ctx, tableItem, itemFieldSlug); err != nil {
		log.Fatalf("could not create index on %s table: %s", tableItem, err)
	}

	return &ItemRepository{c, itemSearchFields}
}

type ItemRepository struct {
	db            *Client
	keywordFields []string
}

func (s *ItemRepository) Find(ctx context.Context, o dotagiftx2.FindOpts) ([]dotagiftx2.Item, error) {
	var res []dotagiftx2.Item
	o.KeywordFields = s.keywordFields
	q := newFindOptsQuery(s.table(), o)
	if err := s.db.list(ctx, q, &res); err != nil {
		return nil, dotagiftx2.NewXError(dotagiftx2.StorageUncaughtErr, err)
	}

	return res, nil
}

func (s *ItemRepository) Count(ctx context.Context, o dotagiftx2.FindOpts) (num int, err error) {
	o = dotagiftx2.FindOpts{
		Keyword:       o.Keyword,
		KeywordFields: s.keywordFields,
		Filter:        o.Filter,
		UserID:        o.UserID,
	}
	q := newFindOptsQuery(s.table(), o)
	err = s.db.one(ctx, q.Count(), &num)
	return
}

func (s *ItemRepository) Get(ctx context.Context, id string) (*dotagiftx2.Item, error) {
	row, _ := s.GetBySlug(ctx, id)
	if row != nil {
		return row, nil
	}

	row = &dotagiftx2.Item{}
	if err := s.db.one(ctx, s.table().Get(id), row); err != nil {
		if errors.Is(err, r.ErrEmptyResult) {
			return nil, dotagiftx2.ItemErrNotFound
		}

		return nil, dotagiftx2.NewXError(dotagiftx2.StorageUncaughtErr, err)
	}

	return row, nil
}

func (s *ItemRepository) GetBySlug(ctx context.Context, slug string) (*dotagiftx2.Item, error) {
	row := &dotagiftx2.Item{}
	q := s.table().GetAllByIndex(itemFieldSlug, slug)
	if err := s.db.one(ctx, q, row); err != nil {
		if errors.Is(err, r.ErrEmptyResult) {
			return nil, dotagiftx2.ItemErrNotFound
		}

		return nil, dotagiftx2.NewXError(dotagiftx2.StorageUncaughtErr, err)
	}

	return row, nil
}

func (s *ItemRepository) Create(ctx context.Context, in *dotagiftx2.Item) error {
	t := now()
	in.CreatedAt = t
	in.UpdatedAt = t
	in.ID = ""
	id, err := s.db.insert(ctx, s.table().Insert(in))
	if err != nil {
		return dotagiftx2.NewXError(dotagiftx2.StorageUncaughtErr, err)
	}
	in.ID = id

	return nil
}

func (s *ItemRepository) Update(ctx context.Context, in *dotagiftx2.Item) error {
	cur, err := s.Get(ctx, in.ID)
	if err != nil {
		return err
	}

	in.UpdatedAt = now()
	err = s.db.update(ctx, s.table().Get(in.ID).Update(in))
	if err != nil {
		return dotagiftx2.NewXError(dotagiftx2.StorageUncaughtErr, err)
	}

	if err := mergo.Merge(in, cur); err != nil {
		return dotagiftx2.NewXError(dotagiftx2.StorageMergeErr, err)
	}

	return nil
}

func (s *ItemRepository) IsItemExist(ctx context.Context, name string) error {
	/*
		r.table('item').filter(function(doc) {
		  return doc.getField('name').match('(?i)^Gothic')
		})
	*/
	q := s.table().Filter(func(t r.Term) r.Term {
		// Matches exact name and non case sensitive.
		return t.Field(itemFieldName).Match(fmt.Sprintf("(?i)^%s$", name))
	})
	var n int
	if err := s.db.one(ctx, q.Count(), &n); err != nil {
		return dotagiftx2.NewXError(dotagiftx2.StorageUncaughtErr, err)
	}

	if n != 0 {
		return dotagiftx2.ItemErrCreateItemExists
	}

	return nil
}

func (s *ItemRepository) AddViewCount(ctx context.Context, id string) error {
	cur, err := s.Get(ctx, id)
	if err != nil {
		return err
	}

	cur.ViewCount++
	if err := s.Update(ctx, cur); err != nil {
		return err
	}

	if err := s.updateCatalogViewCount(ctx, id, cur.ViewCount); err != nil {
		return err
	}

	return nil
}

func (s *ItemRepository) updateCatalogViewCount(ctx context.Context, itemID string, viewCount int) error {
	q := r.Table(tableCatalog).Get(itemID).Update(&dotagiftx2.Catalog{ViewCount: viewCount})
	return s.db.update(ctx, q)
}

func (s *ItemRepository) table() r.Term {
	return r.Table(tableItem)
}
